...
POMO_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, ".."))
TSP_DIR = os.path.abspath(os.path.join(POMO_DIR, ".."))
REPO_DIR = os.path.abspath(os.path.join(TSP_DIR, ".."))

os.chdir(SCRIPT_DIR)
sys.path.insert(0, SCRIPT_DIR)
sys.path.insert(1, POMO_DIR)
sys.path.insert(2, TSP_DIR)
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="PJW training entrypoint: multi-scale distribution-aware POMO fine-tuning."
    )
    parser.add_argument("--exp_name", default="pjw_ft_curriculum_mixed")
    ...
    parser.add_argument(
        ...
        default=os.path.abspath(
            os.path.join(POMO_DIR, "result", "saved_tsp100_model2_longTrain", "checkpoint-3000.pt")
        ),
        help="Baseline or previous checkpoint used for fine-tuning.",
    )
...
def build_params(args):
    first_size = parse_curriculum(args.curriculum)[0]["sizes"][0]
    env_params = {
    ...
    trainer_params = {
        ...
        'problem_size_schedule': parse_curriculum(args.curriculum),
        ...
        'model_load': {
            'enable': bool(args.init_checkpoint),
            'checkpoint_path': os.path.abspath(args.init_checkpoint),
    ...
    return env_params, optimizer_params, trainer_params, logger_params


def dump_config(args, env_params, optimizer_params, trainer_params):
    config = {
        "args": vars(args),
...
def run_validation(args, checkpoint_path):
    ...
    subprocess.run(command, cwd=POMO_DIR, check=True)


def main():
    args = build_parser().parse_args()
    env_params, optimizer_params, trainer_params, logger_params = build_params(args)

    create_logger(**logger_params)
    logger = logging.getLogger("root")
    logger.info("PJW multi-scale distribution-aware fine-tuning")
    logger.info("SCRIPT_DIR: {}".format(SCRIPT_DIR))
    logger.info("POMO_DIR: {}".format(POMO_DIR))
    ...
    dump_config(args, env_params, optimizer_params, trainer_params)
