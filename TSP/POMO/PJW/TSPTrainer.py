...
class TSPTrainer:
    def __init__(self,
        ...
        if model_load['enable']:
            ...
            if model_load.get('load_optimizer', False):
                self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            if model_load.get('load_scheduler', False) and 'scheduler_state_dict' in checkpoint:
                self.scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
            if (not model_load.get('reset_epoch', True)) and 'epoch' in checkpoint:
                self.start_epoch = int(checkpoint['epoch']) + 1
                self.scheduler.last_epoch = int(checkpoint['epoch']) - 1
                if 'result_log' in checkpoint:
                    self.result_log.set_raw_data(checkpoint['result_log'])

            self.logger.info('Saved Model Loaded: {}'.format(checkpoint_fullname))
            self.logger.info('reset_epoch: {}'.format(model_load.get('reset_epoch', True)))
    ...
    def _active_problem_sizes(self, epoch):
        schedule = self.trainer_params['problem_size_schedule']
    ...
    def _sample_distribution(self):
        mix = self.trainer_params['distribution_mix']
        names = list(mix.keys())
