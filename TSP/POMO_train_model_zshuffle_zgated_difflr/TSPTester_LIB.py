...
class TSPTester_LIB:
    def __init__(self, model_params, tester_params):
        ...
        is_gated_ckpt = any(key.startswith('decoder.poly_gate') for key in state_dict)
        ...
        if is_polynet_ckpt:
            self.model_params.update({
                ...
                'use_z_gate': checkpoint_model_params.get(
                    'use_z_gate',
                    self.model_params.get('use_z_gate', is_gated_ckpt),
                ),
            })
        else:
            self.model_params.setdefault('use_polynet', False)
            self.model_params.setdefault('use_z_gate', False)
        ...
        self.logger.info("Gated z residual enabled: {}".format(self.model_params.get('use_z_gate', False)))
    ...
    def _test_one_instance(self, nodes_xy_normalized: torch.Tensor, coords_orig: torch.Tensor, ew_type: str) -> Tuple[float, float]:
        ...
        assert env.pomo_size == problem_size, "Stage-1 Poly-POMO keeps exactly N rollouts per augmentation."
    ...
    def _make_z(self, batch_size: int, rollout_size: int) -> Optional[torch.Tensor]:
        ...
        rollout_idx = torch.arange(rollout_size, device=self.device, dtype=torch.long)
