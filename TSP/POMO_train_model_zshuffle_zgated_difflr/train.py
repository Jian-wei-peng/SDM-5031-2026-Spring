...
def build_parser():
    parser = argparse.ArgumentParser(
        description="POMO final combined variant with z-shuffle, gated residual, and differential LR."
    )
    parser.add_argument("--exp_name", default="pomo_train_model_zshuffle_zgated_difflr")
    ...
    parser.add_argument("--use_differential_lr", type=str2bool, default=True)
    parser.add_argument("--backbone_lr", type=float, default=1e-6)
    parser.add_argument("--poly_lr", type=float, default=1e-5)
    ...
    parser.add_argument("--use_z_gate", type=str2bool, default=True)
    parser.add_argument(
        "--shuffle_z_train",
        type=str2bool,
        default=True,
        help="Randomly permute the assignment between rollout index and z code during training.",
    )
    return parser
...
def build_model_params(args):
    if not args.force_first_move:
        raise ValueError("POMO_train_model_zshuffle_zgated_difflr stage 1 supports only --force_first_move true.")
...
def build_params(args):
    ...
    trainer_params = {
        ...
        'shuffle_z_train': args.shuffle_z_train,
        'logging': {
...
def main():
    ...
    logger.info("POMO final z-shuffle + gated residual + differential-lr training")
