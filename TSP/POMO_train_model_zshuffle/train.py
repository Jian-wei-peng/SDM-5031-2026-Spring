...
def build_parser():
    parser = argparse.ArgumentParser(
        description="POMO train+model ablation with multi-scale fine-tuning and random z assignment."
    )
    parser.add_argument("--exp_name", default="pomo_train_model_zshuffle_stage1")
...
def build_model_params(args):
    if not args.force_first_move:
        raise ValueError("POMO_train_model_zshuffle stage 1 supports only --force_first_move true.")
...
def main():
    ...
    logger.info("POMO train+model z-shuffle training")
