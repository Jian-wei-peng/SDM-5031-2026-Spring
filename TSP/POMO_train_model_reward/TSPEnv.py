...
class TSPEnv:
    ...
    def step(self, selected, lib_mode: bool = False):
        ...
        if done:
            # === 方案二：改进的POMO奖励 ===
            travel_distances = self._get_travel_distance(lib_mode=lib_mode)

            # 1. 基础负距离奖励
            base_reward = -travel_distances

            # 2. 添加POMO内部基线（同一batch内不同POMO轨迹的均值）
            pomo_mean = travel_distances.mean(dim=1, keepdim=True)
            advantage = -(travel_distances - pomo_mean)  # 优势函数

            # 3. 可选：添加问题难度自适应缩放
            problem_scale = travel_distances.max(dim=1, keepdim=True)[0] - travel_distances.min(dim=1, keepdim=True)[0]
            normalized_advantage = advantage / (problem_scale + 1e-8)

            reward = base_reward + 0.5 * normalized_advantage.squeeze(1)
        else:
            reward = None
