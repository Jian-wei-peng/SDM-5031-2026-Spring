...
def build_parser():
    parser = argparse.ArgumentParser(
        description="POMO train+model ablation with gated z-conditioned residual."
    )
    parser.add_argument("--exp_name", default="pomo_train_model_zgated")
...
def build_model_params(args):
    if not args.force_first_move:
        raise ValueError("POMO_train_model_zgated stage 1 supports only --force_first_move true.")
...
def main():
    ...
    logger.info("POMO train+model gated residual training")
