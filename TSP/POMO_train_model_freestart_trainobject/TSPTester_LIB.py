...
class TSPTester_LIB:
    def __init__(self, model_params, tester_params):
        ...
        checkpoint_trainer_params = checkpoint.get('trainer_params', {})
        self.checkpoint_K = checkpoint_trainer_params.get(
            'K',
            checkpoint_model_params.get('K', tester_params.get('K')),
        )
        if is_polynet_ckpt:
            self.model_params.update({
                'use_polynet': True,
                'z_dim': checkpoint_model_params.get('z_dim', self.model_params.get('z_dim', 16)),
                'poly_embedding_dim': checkpoint_model_params.get(
                    'poly_embedding_dim',
                    self.model_params.get('poly_embedding_dim', 256),
                ),
                'force_first_move': checkpoint_model_params.get(
                    'force_first_move',
                    self.model_params.get('force_first_move', True),
                ),
            })
        else:
            self.model_params.setdefault('use_polynet', False)
        ...
        self.logger.info("PolyNet stage-1 decoder enabled: {}".format(self.model_params.get('use_polynet', False)))
    ...
    def _test_one_instance(self, nodes_xy_normalized: torch.Tensor, coords_orig: torch.Tensor, ew_type: str) -> Tuple[float, float]:
        ...
        if not self.model_params.get('force_first_move', True):
            env.pomo_size = self._strategy_count(problem_size)
            env.env_params['pomo_size'] = env.pomo_size
        ...
        with torch.no_grad():
            ...
            z = self._make_z(effective_batch, env.pomo_size)
    ...
    def _strategy_count(self, problem_size: int) -> int:
        if self.checkpoint_K is not None:
            return int(self.checkpoint_K)
        return int(self.tester_params.get('K') or problem_size)

    def _make_z(self, batch_size: int, rollout_size: int) -> Optional[torch.Tensor]:
        ...
        if code_count > 1_000_000:
            return torch.randint(0, 2, (batch_size, rollout_size, z_dim), device=self.device).float()
        replacement = rollout_size > code_count
        weights = torch.ones(batch_size, code_count, device=self.device)
        z_idx = torch.multinomial(weights, rollout_size, replacement=replacement)
        return ((z_idx[:, :, None] >> bit_idx[None, None, :]) & 1).float()
