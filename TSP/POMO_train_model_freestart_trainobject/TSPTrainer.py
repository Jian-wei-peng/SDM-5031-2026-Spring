...
class TSPTrainer:
    ...
    def _load_checkpoint_if_needed(self, model_load):
        ...
        has_freestart_ckpt = 'start_q1' in state_dict and 'start_last_node' in state_dict
        use_polynet = self.model_params.get('use_polynet', False)
        force_first_move = self.model_params.get('force_first_move', True)
        strict_load = not (use_polynet or not force_first_move)
        if use_polynet and is_polynet_ckpt and (force_first_move or has_freestart_ckpt):
            strict_load = True
        ...
        if not strict_load:
            self.logger.info(
                'non_strict_load missing_keys: {}, unexpected_keys: {}'.format(
    ...
    def _train_one_batch(self, batch_size, problem_size, distribution):
        ...
        train_objective = self.trainer_params.get('train_objective', 'best_of_k')
        if train_objective == 'pomo':
    ...
    def _load_training_problems(self, batch_size, problem_size, distribution):
        rollout_size = self._rollout_size_for_problem(problem_size)
        self.env.problem_size = problem_size
        self.env.pomo_size = rollout_size
        self.env.env_params['problem_size'] = problem_size
        self.env.env_params['pomo_size'] = rollout_size
        ...
        device = self.env.problems.device
        self.env.BATCH_IDX = torch.arange(batch_size, device=device)[:, None].expand(batch_size, rollout_size)
        self.env.POMO_IDX = torch.arange(rollout_size, device=device)[None, :].expand(batch_size, rollout_size)

    def _rollout_size_for_problem(self, problem_size):
        if self.model_params.get('force_first_move', True):
            return problem_size
        return self.trainer_params.get('K', self.trainer_params.get('strategy_count', problem_size))
    ...
    def _make_z(self, batch_size, rollout_size):
        ...
        if code_count > 1_000_000:
            return torch.randint(0, 2, (batch_size, rollout_size, z_dim), device=device).float()
        replacement = rollout_size > code_count
        weights = torch.ones(batch_size, code_count, device=device)
    ...
    def _save_checkpoint_images(self, epoch):
        try:
            self.logger.info("Saving checkpoint log_image")
