"""
TSP Model with PPO Support - 添加Critic网络用于PPO训练

修改自原始TSPModel.py，添加价值函数头
"""
...
class TSPModel_PPO(nn.Module):
    """
    扩展TSPModel，添加Critic网络用于PPO训练
    
    主要新增：
    - critic_head: 价值函数估计网络
    - get_value(): 计算状态价值
    """
    ...
    def get_value(self, state=None):
        ...
        Args:
            state: 当前状态（可选，用于更复杂的价值估计）
        
        Returns:
            value: (batch,) 或 (batch, pomo) 的价值估计
        """
        # 方法1：简单平均（推荐用于快速实验）
